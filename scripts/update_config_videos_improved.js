...
function main() {
  ...
  config.assets.forEach((asset, index) => {
    if (asset.type === "VIDEO") {
      ...
      let expectedMovFile = null;

      // Strategy 1: Try to map using CSV (for original filenames like DF25_000257123.mp4)
      const newBaseName = nameMap[originalFileName];
      if (newBaseName) {
        expectedMovFile = `${newBaseName}.mov`;
      } else {
        // Strategy 2: For files already using new naming format, just change extension
        // e.g., Phase1_Video_R1300RS_close-up_no-text_square_1080x1350.mp4 -> .mov
        const baseName = path.parse(originalFileName).name;
        expectedMovFile = `${baseName}.mov`;
      }

      // Check if the .mov file exists
      if (expectedMovFile && availableMovFiles.has(expectedMovFile)) {
        ...
        console.log(
          `Skipped ${asset.id}: .mov file not found (${
            expectedMovFile || "could not determine filename"
          })`
        );
        skippedCount++;
      }
