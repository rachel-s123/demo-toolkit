...
const SUPPORTED_EXTENSIONS = [".mov", ".mp4"];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (columns.length > Math.max(originalNameIndex, newNameIndex)) {
      ...
      if (originalNameWithExt && newNameWithExt) {
        // Store both the base name and the extension
        const originalParsed = path.parse(originalNameWithExt);
        const newParsed = path.parse(newNameWithExt);
        map[originalParsed.name] = {
          newName: newParsed.name,
          originalExt: originalParsed.ext,
          newExt: newParsed.ext
        };
      }
    }
  }
...
function main() {
  ...
  const videoFiles = filesInDir.filter(
    (file) => SUPPORTED_EXTENSIONS.includes(path.extname(file).toLowerCase())
  );
  const existingTargetNames = new Set(videoFiles); // Keep track of all names currently in the folder

  for (const currentFileName of videoFiles) {
    const currentBaseName = path.parse(currentFileName).name;
    const currentExt = path.extname(currentFileName).toLowerCase();
    const mapping = nameMap[currentBaseName];

    if (mapping) {
      // Use the new extension from the mapping, or keep the original if not specified
      const targetExt = mapping.newExt || currentExt;
      const targetFileName = `${mapping.newName}${targetExt}`;
